...
function Keuangan({ idBsu }) {
  ...
  } = useFetch("/api/keuangan/pengeluaran/getPengeluaran", {
    method: "GET",
  });
  ...
  } = useFetch("/api/keuangan/pemasukan/getPemasukan", {
    method: "GET",
  });
  ...
  } = useFetch("/api/keuangan/pemasukan/getPenjualan", {
    method: "GET",
  });
  ...
  useEffect(() => {
    ...
    if (dataPemasukan || dataPenjualan) {
      const totalPemasukanCalc = (Array.isArray(dataPemasukan?.data)
        ? dataPemasukan.data.reduce((acc, item) => acc + (item.saldo || 0), 0)
        : 0) + (Array.isArray(dataPenjualan?.data)
  ...
  useEffect(() => {
    if (dataKeuangan && dataPemasukan && dataPenjualan) {
      ...
      const combinedData = [...pengeluaranData, ...pemasukanData, ...penjualanData];
  ...
  if (errorKeuangan || errorPemasukan || errorPenjualan) {
    toast({
      variant: "destructive",
      title: "Gagal!",
      description: errorKeuangan?.message || errorPemasukan?.message || errorPenjualan?.message,
    });
  }
